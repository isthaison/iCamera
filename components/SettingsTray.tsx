    ...
    <div className="absolute top-14 left-0 right-0 z-40 bg-black/90 px-6 py-4 animate-in slide-in-from-top duration-300 backdrop-blur-xl border-b border-white/10">
      <div className="space-y-6">
        <div className="flex justify-around items-center">
          <button onClick={() => store.setShowGrid(!store.showGrid)} className="flex flex-col items-center gap-1">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${store.showGrid ? 'bg-yellow-400 text-black' : 'bg-white/10 text-white'}`}>
          ...
          <div className="flex gap-2 bg-white/5 p-1 rounded-full self-center">
            {(['4:3', '16:9', '1:1'] as const).map(ratio => (
              <button
                ...
                className={`px-4 py-1 rounded-full text-[10px] font-bold transition-all ${store.aspectRatio === ratio ? 'bg-white text-black' : 'text-white/40'}`}
              >
                {ratio}
              </button>
            ))}
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex gap-4 overflow-x-auto no-scrollbar py-2 px-2">
            {filters.map(f => (
              ...
              >
                <div className={`w-12 h-12 rounded-lg border-2 transition-all ${store.filter === f ? 'border-yellow-400 scale-110 shadow-lg shadow-yellow-400/20' : 'border-white/10 opacity-60'}`}>
                ...
                <span className={`text-[8px] font-bold ${store.filter === f ? 'text-yellow-400' : 'text-white/40'}`}>{f}</span>
              </button>
            ))}
          </div>
        </div>
