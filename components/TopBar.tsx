...
const TopBar: React.FC = () => {
  ...
  return (
    <div className={`absolute top-0 left-0 right-0 z-[60] transition-all duration-500 pt-safe ${store.isSettingsOpen ? 'bg-black/40 backdrop-blur-md' : 'bg-transparent'}`}>
      <div className="flex items-center justify-between px-6 h-14">
        {/* Quick Indicators (Left) */}
        <div className="flex items-center gap-4">
          {store.torch && !isFrontCamera && (
        ...
        >
          <div className={`transition-transform duration-500 ${store.isSettingsOpen ? 'rotate-180' : 'rotate-0'}`}>
        ...
        <div className="flex items-center gap-3">
          {store.timer > 0 && (
            <div className="flex items-center gap-1 text-yellow-400">
              ...
              <span className="text-[10px] font-bold">{store.timer}s</span>
            </div>
          )}
        </div>
      </div>
